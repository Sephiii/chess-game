export class PieceSprite {
    private static sprites: { [key: string]: string } = {};
    ...
    static async loadSprites(): Promise<void> {
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="270" height="90" viewBox="0 0 270 90">
            ...
            <!-- Pièces noires (copies avec remplissage noir) -->
    ...
    static drawPiece(ctx: CanvasRenderingContext2D, spriteName: string, x: number, y: number): void {
        if (!this.spriteSheet) return;

        const spriteX = this.getSpriteX(spriteName);
        const spriteY = spriteName.startsWith('black') ? this.SPRITE_SIZE : 0;
    ...
    private static getSpriteX(spriteName: string): number {
        const types = {
            'king': 0,
            'queen': 1,
            'bishop': 2,
        ...
        const pieceType = spriteName.split('-')[1] as keyof typeof types;
        return types[pieceType] * this.SPRITE_SIZE;
    }
}